...
def _same_uploaded_category(product: dict[str, Any], competitor: dict[str, Any]) -> bool:
    ...
    if competitor_category_id and product_category_ids:
        ...
        if product_category and competitor_category:
            same_category = product_category == competitor_category
        else:
            same_category = True # Fallback if no category info

    if not same_category:
        return False

    # 2. Granular Pack-Size/Type Filter
    p_title = (product.get("title") or product.get("asin") or "").lower()
    c_title = (competitor.get("title") or competitor.get("asin") or "").lower()
    
    # Define Scale Groups (Relaxed for maximum inclusion)
    def get_scale(title):
        # We no longer strictly separate bulk vs standard to capture all variations
        return "all"

    p_scale = get_scale(p_title)
    c_scale = get_scale(c_title)

    # No exclusions - let the user see all variations
    return True
