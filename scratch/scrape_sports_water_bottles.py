...
def run_sports_water_bottles_scrape():
    ...
    account_id = "s2c_test"
    marketplace = "UAE"
    category_id = "17007680031" # Sports Water Bottles category ID in Amazon.ae
    category_name = "Sports Water Bottles"

    print("\n==================================================")
    print("   SADDL SCRAPER: SPORTS WATER BOTTLES PIPELINE  ")
    print("==================================================")

    # 1. Register s2c_org and s2c_test client in Supabase pb_clients so it appears in the UI
    print("\n[Step 1/7] Ensuring organization and client registration in Supabase...")
    try:
        ...
        print("[OK] Client 's2c_test' successfully registered/updated under 's2c_org'.")
    ...
    # Filter products specifically for the Sports Water Bottles category
    ...
    for p in target_products:
        ...
        listing_price = live_prices.get(asin) or 69.0
        try:
    ...
    # 3. Trigger live Amazon category search scrape on Apify
    print(f"\n[Step 4/7] Triggering live Apify category scrape on Amazon.ae for '{category_name}'...")
    apify_client = ApifyClient(apify_token)
    
    # We query Amazon.ae using search query "Sports Water Bottles"
    run_input = {
        "categoryOrProductUrls": [{"url": "https://www.amazon.ae/s?k=Sports+Water+Bottles"}],
        "maxItems": 100, # Large competitor pool size
        "proxyConfiguration": {
            "useApifyProxy": True,
    ...
    for parent_asin, child_products in parent_asin_map.items():
        ...
        print(f"   Matches {len(filtered_competitors)} relevant competitor sports water bottles (Reference Keyword: '{ref_name or 'Unfiltered Category'}')")
        ...
        child_prices = [float(live_prices.get(asin) or 69.0) for asin in child_asins]
        ...
        try:
            ...
            print(f"   [RESULT] Current Avg={avg_listing_price} AED | Recommended={rec.get('recommended_price')} AED ({rec.get('action')})")
            print(f"   [RESULT] Benchmark Range: Floor={snap.get('floor_price')} AED | Median={snap.get('median_price')} AED | Ceiling={snap.get('ceiling_price')} AED | Position Percentile={snap.get('percentile_rank')}%")
    ...
    print("\n==================================================")
    print("  [SUCCESS] PIPELINE RUN COMPLETE: Sports Water Bottles Scraped!")
    print("==================================================")
...
if __name__ == "__main__":
    run_sports_water_bottles_scrape()
